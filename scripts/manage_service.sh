...
usage() {
  echo "Usage: ./scripts/manage_service.sh [install|uninstall|start|stop|status|restart]"
}
...
ExecStart=/usr/bin/env uvicorn tools.repo_orchestrator.main:app --host 127.0.0.1 --port 9325
Restart=on-failure
RestartSec=3
