...
class TestPromptInjectionAttacks:
            ...
            if response.status_code == 200:
                ...
                for token in TOKENS:
                    assert token not in content, \
                        f"Token leaked in response from {file_path}"
    ...
    def test_diff_endpoint_secret_leakage(self):
        ...
        if response.status_code == 200:
            ...
            for token in TOKENS:
                assert token not in diff_content, \
                    "Token exposed in diff output"
    ...
    def test_bulk_file_enumeration(self):
        ...
        if response.status_code == 200:
            ...
            assert len(files) < 3000, \
                "Tree endpoint should limit number of files returned"
    ...
    def test_large_file_read_prevention(self):
        ...
        if response.status_code == 200:
            ...
            assert line_count <= 500, \
                "File endpoint should limit number of lines returned"
    ...
    def test_recursive_search_limits(self):
        ...
        if response.status_code == 200:
            ...
            assert len(results) <= 50, \
                "Search should limit number of results"
...
def generate_report(test_results):
    ...
    report = {
        ...
        "vulnerabilities_detected": [
            r for r in test_results if not r["passed"]
        ],
