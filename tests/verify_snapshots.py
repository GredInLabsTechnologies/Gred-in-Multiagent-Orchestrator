...
def verify():
    ...
    r_get = requests.get(f"{ORCH_URL}/file", headers=headers, params={"path": test_file}, timeout=10)
    ...
    r_post = requests.post(f"{ORCH_URL}/file", headers=headers, json={"path": "test.txt", "content": "fail"}, timeout=10)
    ...
    r_ui = requests.post(f"{ORCH_URL}/ui/repos/select", headers=headers, params={"path": "."}, timeout=10)
    print(f"    Status: {r_ui.status_code}")
    if r_ui.status_code == 404:
        print("    [PASS] UI POST endpoint is disabled.")
    else:
        print(f"    [FAIL] UI POST endpoint still accessible.")
